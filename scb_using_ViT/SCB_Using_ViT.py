...
@dataclass
class DatasetConfig:
    name: str = "scb_dataset"
    path: str = "./SCB_dataset"
    classes: Tuple[str, ...] = (
        "hand-raising",
        "reading",
        "writing",
...
@dataclass
class LoggingConfig:
    log_dir: str = "./runs/scb"
    checkpoint_dir: str = "./runs/scb/weights"
...
class SCBDataset(Dataset):
    def __init__(
        ...
        self.class_names = list(CFG.dataset.classes)
        self.num_classes = CFG.dataset.num_classes
        self.images, self.labels = self._load_data_paths()
        self.transform = self._build_transforms()

    def _load_data_paths(self) -> Tuple[List[str], List[Optional[str]]]:
        images_dir = os.path.join(self.dataset_path, "images", self.split)
        labels_dir = os.path.join(self.dataset_path, "labels", self.split)
...
class HybridTrainer:
    def __init__(self, cfg: GlobalConfig):
        ...
        self.results_csv_path = os.path.join(cfg.logging.log_dir, "results1.csv")
...
def run_inference_single_image(
    ...
    checkpoint_path: str,
    image_path: str,
    out_path: str = "scb_example_output.jpg",
):
...
def main_infer_example():
    ...
    if not os.path.exists(img_path):
        ...
        return
    run_inference_single_image(CFG, ckpt_path, img_path, out_path=os.path.join(CFG.logging.log_dir, "scb_example_output.jpg"))
